...
def load_table( uploaded_file: io.BytesIO | None, uploaded_name: str | None,
...
try:
    df = load_table( uploaded_file=uploaded, uploaded_name=uploaded.name if uploaded else None,
...
if dup_count > 0:
		st.info( 'Duplicate headers detected. When a single column is requested by name, '
    ...
    if not cols:
        ...
        if chosen:
            ...
            for i, c in enumerate(chosen):
                ...
                ax.hist( s, bins=bins, alpha=0.40,
                    label=c, color=PALETTE[i % len(PALETTE)], edgecolor='black',
                    linewidth=0.5,  )
        ...
        if len(selected) >= 2:
            ...
            if Xdf.shape[0] < 10:
                ...
                for lab in np.unique(labels):
                    ...
                    ax.scatter( comps[idx, 0], comps[idx, 1], label=f'Cluster {lab}',
                        marker=marker, edgecolor='black', linewidth=0.4, alpha=0.90,
                        c=color, )
...
with tabs[8]:
    if not HAS_STATSMODELS:
        ...
        period_candidates = [ c for c in df.columns
            ...
            if value_cols:
                ...
                for i, col in enumerate(value_cols):
                    ...
                    ax.plot( x, y, label=col,
                        ...
                        marker=MARKERS[i % len(MARKERS)], linewidth=1.6, alpha=0.95, )
...
with tabs[9]:
    st.download_button( label='Download CSV',
        ...
        file_name='balance_projection_export.csv', mime='text/csv',  )
